...
def beale(x, y):
    h = np.power((1.5 - x + x*y), 2) + np.power((2.25 - x + x*(np.power(y, 2))),
                                                2) + np.power((2.625 - x + x*(np.power(y, 3))), 2)
    a = 0.000000000000000001
    return 1/(h + a)
...
def bohanchevsky(x, y):
    h = np.power(x, 2) + 2 * np.power(y, 2) - 0.3 * \
...
def dropWave(x, y):
    h = -1 * (1 + np.cos(12 * np.sqrt(np.power(x, 2)+np.power(y, 2)))
              ) / (0.5 * (np.power(x, 2) + np.power(y, 2)) + 2)
    a = 0.000000000000000001
    return 1/(h + a)
...
def easom(x, y):
    h = -1*np.cos(x)*np.cos(y)*np.exp(-1 *
                                      np.power((x - np.pi), 2) - np.power((y - np.pi), 2))
    a = 0.000000000000000001
    return 1/(h + a)
...
def egg_create(x, y):
    h = np.power(x, 2) + np.power(y, 2) + 25 * \
        (np.power(np.sin(x), 2) + np.power(np.sin(y), 2))
    a = 0.000000000000000001
    return 1/(h + a)
...
def schafferN3(x, y):
    numeratorcomp = np.power(
        (np.sin(np.cos(np.abs((np.power(x, 2) - np.power(y, 2)))))), 2) - 0.5
    denominatorcomp = np.power(
...
def bird(x, y):
    h = np.sin(x)*np.exp(np.power((1-np.cos(y)), 2)) + np.cos(y) * \
        np.exp(np.power((1-np.sin(x)), 2)) + np.power((x-y), 2)
    a = 0.000000000000000001
    return 1/(h + a)
...
def bartelsConn(x, y):
    h = np.abs(np.power(x, 2) + np.power(y, 2) + x*y) + \
...
def ackleyn3(x, y):
    h = -1*200*np.exp(-1*0.02 * np.sqrt(np.power(x, 2)) +
                      np.power(y, 2)) + 5*np.exp(np.cos(3*x) + np.sin(3*y))
    a = 0.000000000000000001
    return 1/(h + a)
